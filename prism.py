...
mesh = stl.mesh.Mesh(np.zeros(n_prisms**2 * n_facets * 3, dtype=stl.mesh.Mesh.dtype))
...
for i in range(n_prisms):
    for j in range(n_prisms):
        ...
        for k in range(n_facets):
            ...
            mesh.vectors[i * n_prisms * n_facets + j * n_facets + k] = np.array([v1, v2, v3])

# Save the mesh as STL file
mesh.save('fresnel_lens.stl')
